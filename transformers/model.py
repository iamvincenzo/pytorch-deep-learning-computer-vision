...
class Transformer(nn.Module):
    def __init__(self, num_encoder_blocks: int, num_heads: int, embed_dim: int, n_classes: int, dropout: float) -> None:
        """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.dropout(x)
        x = self.global_avg_pool(x.permute(0, 2, 1)).squeeze(-1) # gloabl_avg_pool on seq_len dim
